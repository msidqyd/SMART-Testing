...
def SMART():
    ...
    def make_engine():
        ...
        Encrypt = db_secrets.get("Encrypt", "")
        TrustServerCertificate = db_secrets.get("TrustServerCertificate", "")
        ConnectionTimeout = db_secrets.get("ConnectionTimeout", "")
    ...
    def _parse_hhmmss_to_seconds(x):
        ...
        if digits.isdigit() and 1 <= len(digits) <= 6:
            digits = digits.zfill(6); h, m_, s_ = int(digits[:2]), int(digits[2:4]), int(digits[4:6])
    ...
    with st.sidebar.form("filters", clear_on_submit=False):
        ...
        # Wrap first call so we can surface DBAPI errors clearly
        try:
        ...
        if amo_default not in amo_opts: amo_default = "All"
        ...
        if wh_default not in wh_opts: wh_default = "All"
        ...
        if spv_default not in spv_opts: spv_default = "All"
        ...
        if region_default not in region_opts: region_default = "All"
    ...
    def build_where(div_, amo_, wh_, spv_, region_, start_, end_):
        parts, p = [], {}
        if div_    != "All" and DIVISION_COL: parts.append(f" AND {DIVISION_COL} = :division"); p["division"] = div_
        if amo_    != "All" and AMO_COL:      parts.append(f" AND {AMO_COL} = :amo");           p["amo"]      = amo_
        if wh_     != "All" and WH_COL:       parts.append(f" AND {WH_COL} = :wh");             p["wh"]       = wh_
        if spv_    != "All" and SPV_COL:      parts.append(f" AND {SPV_COL} = :spv");           p["spv"]      = spv_
        if region_ != "All" and REGION_COL:   parts.append(f" AND {REGION_COL} = :region");     p["region"]   = region_
    ...
    if TIME_COL:   rename_map[TIME_COL]   = "Travel Time (HH:MM:SS)"
    if DIST_COL:   rename_map[DIST_COL]   = "Distance (m)"
    if RADIUS_COL: rename_map[RADIUS_COL] = "Radius MNT (m)"
    ...
    # ============================ Route Tracking (summary from SQL) ============================
    ...
    if rs.get("AvgSecs") is not None and not pd.isna(rs["AvgSecs"]):
        total_sec = int(rs["AvgSecs"]); h = total_sec//3600; m = (total_sec%3600)//60; s = total_sec%60
        ...
        if source_col:
            ...
            if len(secs):
                mean = int(np.mean(secs)); h=mean//3600; m=(mean%3600)//60; s=mean%60
                avg_tt = f"{h:02d}:{m:02d}:{s:02d}"
    ...
    try:
        if route_summary["First Outlet"] and route_summary["Last Outlet"]:
            route_summary["Working Time"] = str(pd.to_datetime(route_summary["Last Outlet"]) - pd.to_datetime(route_summary["First Outlet"]))
    except Exception:
        pass
    ...
    def style_df(df_):
        ...
        if "NPL Pack" in df_.columns:       sty = sty.applymap(lambda v: highlight_cells(v, "NPL Pack"), subset=["NPL Pack"])
        if "Radius MNT (m)" in df_.columns: sty = sty.applymap(lambda v: highlight_cells(v, "Radius MNT (m)"), subset=["Radius MNT (m)"])
        if "Distance (m)" in df_.columns:   sty = sty.applymap(lambda v: highlight_cells(v, "Distance (m)"), subset=["Distance (m)"])
        fmt = {}
        if "Distance (m)" in df_.columns:   fmt["Distance (m)"]   = "{:,.0f}"
        if "Radius MNT (m)" in df_.columns: fmt["Radius MNT (m)"] = "{:,.0f}"
    ...
    if missing:
        ...
        if df_map.empty:
            ...
            def add_number_label(lat, lon, text):
                if not text:
                    return
                # offset label slightly above the marker; scale by size
            ...
            # salesman markers (logo or big red dot) + labels
            ...
            for (idx, row), num in zip(df_map.iterrows(), seq_vals):
                if pd.notna(row["Latitude"]) and pd.notna(row["Longitude"]):
                    ...
                    popup_html = (
                        f"<b>Seq:</b> {label}<br>"
                        f"<b>Store:</b> {row.get('Store Name', '')}<br>"  # Display Store Name (Company Name)
            ...
            all_pts = []
            all_pts += [[r["Latitude"], r["Longitude"]] for _, r in df_map.iterrows() if pd.notna(r["Latitude"]) and pd.notna(r["Longitude"])]
            if {"Outlet_Latitude", "Outlet_Longitude"}.issubset(df.columns):
